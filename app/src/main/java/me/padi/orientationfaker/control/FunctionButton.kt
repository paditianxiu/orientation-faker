package me.padi.orientationfaker.control
...
sealed interface FunctionButton : Serializable {
    ...
    enum class OrientationButton(
        ...
        companion object {
            const val PREFIX = "O"
            fun of(name: String): OrientationButton? = entries.find { it.name == name }
        }
    }
    ...
    enum class LauncherButton : FunctionButton {
        ...
        override val index: Int = ordinal + OrientationButton.entries.size
        override fun toString(): String = PREFIX + PREFIX_DELIMITER + name

        companion object {
            const val PREFIX = "L"
            fun of(name: String): LauncherButton? = entries.find { it.name == name }
        }
    }
    ...
    companion object {
        ...
        fun all(): List<FunctionButton> =
            OrientationButton.entries + LauncherButton.entries
        ...
        private fun Orientation.toFunctionButton(): FunctionButton? =
            OrientationButton.entries.find { it.orientation == this }
...
fun List<FunctionButton>.mapOrientation(): List<Orientation> =
    mapNotNull { (it as? FunctionButton.OrientationButton)?.orientation }
